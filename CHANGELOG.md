...
- **Keyguard Designer is moving to a web address of its own, and the app can now save a copy of your settings.** Once you have a project open, a notice at the top of the app asks you to save one; it appears each time until you do, and **Not now** puts it off until your next visit. The copy is written into that project folder as **keyguard-settings.json** — beside the files it belongs with, and where the app can find it for you afterwards, so saving is a single click. Your settings are held by your browser against the app's web address rather than in your project folder, so a saved copy is what lets you carry them to the new address, to another computer, or back after a browser clear-out.

- **When the app moves, it takes your settings with it.** Once the new address is ready, opening the app carries your settings across and lands you there; you'll be asked to open your project again, because permission to read a folder on your computer can't move between web addresses — nothing in the folder changes. If your settings don't make the crossing, opening your project puts them back from the copy saved there, without you having to find anything. A reminder then appears with a link to a short page explaining how to update your bookmark and replace the app's icon.
...
- **Highlighted case additions are pink again, not purple.** Marking a case addition with `#` draws it in the highlight colour on top of the keyguard — but because the highlight was see-through, its colour mixed with the colour of the keyguard underneath it. Added plastic sits in the same place as the keyguard body, so the mix showed up purple over a blue keyguard and grey-green over a turquoise one, and only looked pink where a highlight happened to fall over an opening. Highlights are now drawn solid, so they read the same pink wherever they land and whatever viewport colour you've chosen. The one place a see-through highlight still makes sense is unchanged: when you display a keyguard inside its frame, the ghost keyguard stays translucent so you can see the frame through it.
